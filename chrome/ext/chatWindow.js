...
function createOptionHTML() { // requires this element.function to feed an jdat.rgb color, jdat.id (element id), jdat.header_html
  ...
  var cont = ele('div');
  a(cont, [    ['id', jdat.id],    ['style', `position: fixed; top: ${rect.top}px; left: ${(rect.left)}px; z-index: ${new Date().getTime()}; width: ${jdat.width}px; height: ${jdat.height}px; border: 1px solid ${jdat.rgb}; border-radius: 0.4em; background: transparent; opacity: 1;`]  ]);
...
function createInitChatButton(){
  ...
  if(room_header_elm){
    ...
    var cont = ele('div');
    a(cont,[['id','custom_chat_program'],['jdat',`${JSON.stringify({width: '500px', height: '500px', id:'custom_chat_program',rgb:'#007862',header_html:'Multi Mod',channel: reg(/(?<=twitch\.tv\/)\w+/.exec(window.location.href),0)})}`],['style',`position: fixed; top: ${rect.top+10}px; left: ${rect.left+90}px; transition: all 333ms; padding: 2px; width: 30px; height: 30px; z-index: ${new Date().getTime()}`]]);
    ...
    cont.onmouseleave = () => { path.setAttribute('fill', 'none'); cont.style.width = '30px'; cont.style.height = '30px';  cont.style.left = `${rect.left+90}px`;}
//     cont.onmousedown = () => { path.setAttribute('stroke', 'none'); }
//     cont.onmouseup = () => { path.setAttribute('stroke', '#007862'); cont.style.width = '500px'; cont.style.height = '500px'; cont.style.left = `${rect.left-140}px`; }
